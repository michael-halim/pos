...
from generals.constants import (
    ...
    PERM_R_CASHIER_SALES_REPORT, DATE_FORMAT_DDMMYYYY
) 
...
class CashierSalesReportWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        # Load the UI file
        self.ui = uic.loadUi(resource_path('ui/cashier_sales_report.ui'), self)
        ...
        self.ui.filter_cashier_sales_detail_transactions_input.textChanged.connect(self.filter_detail_transactions)
        ...
        # Set date input
        self.ui.start_date_cashier_sales_input.setDate(datetime.now() - timedelta(days=1))
        self.ui.end_date_cashier_sales_input.setDate(datetime.now())

        self.ui.start_date_cashier_sales_input.setDisplayFormat(DATE_FORMAT_DDMMYYYY)
        self.ui.end_date_cashier_sales_input.setDisplayFormat(DATE_FORMAT_DDMMYYYY)
    ...
    def showEvent(self, event):
        ...
        if not self.permission_manager.has_permission(PERM_R_CASHIER_SALES_REPORT):
            POSMessageBox.error(self, title=PERM_DENIED, message=ERR_PERM_R_CASHIER_SALES_REPORT)
            self.close()
            return
    ...
    def show_transactions_data(self):
        if not self.permission_manager.has_permission(PERM_R_CASHIER_SALES_REPORT):
            POSMessageBox.warning(self, title=PERM_DENIED, message=ERR_PERM_R_CASHIER_SALES_REPORT)
            return
        ...
        # Get Dates
        start_date = datetime.strptime(self.ui.start_date_cashier_sales_input.date().toString(DATE_FORMAT_DDMMYYYY), '%d/%m/%Y')
        end_date = datetime.strptime(self.ui.end_date_cashier_sales_input.date().toString(DATE_FORMAT_DDMMYYYY), '%d/%m/%Y')

        # Get search text if any
        search_text = self.ui.filter_cashier_sales_transactions_input.text().strip()
        search_text = search_text if search_text != '' else None

        # Get all transactions
        transactions_result = self.cashier_sales_report_service.get_transactions_list(
    ...
    def filter_detail_transactions(self):
        search_text = self.ui.filter_cashier_sales_detail_transactions_input.text().lower()
    ...
    def on_transaction_selected(self):
        ...
        if selected_rows:
            ...
            dt_results = self.cashier_sales_report_service.get_detail_transactions_list(selected_transaction)
